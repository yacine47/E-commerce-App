import 'package:flutter/material.dart';

abstract class MyColors {
  static const Color primaryColor = Color(0xffFF204E);
  static const Color primaryColorWithOpacity = Color(0xffffbcca);
  static Color primaryColor2 = const Color(0xff00CDBD);
  static Color primaryColor3 = const Color(0xff00a1e4);
  static Color subTitleColor = const Color(0xff707070);
  static Color iconColorNavigationBar = const Color(0xff767676);
  static Color lighBackgroundColor = const Color(0xffE0E0E0);
  static Color hintColorTextField = const Color(0xff9E9E9E);
  static const Color textFieldColor = Color(0xffFAFAFA);
  static Color borderCategoryColor = const Color(0xffEDEDED);
  static Color starColor = const Color(0xffE6BB66);
  static Color shimmerBaseColor = Colors.grey.withOpacity(0.05);
  static Color shimmerHighlightColor = Colors.grey.withOpacity(0.2);
  static Color gapColor = const Color(0xffF5F5F5);
}


// : Colors.grey.withOpacity(0.05),
//         : Colors.grey.withOpacity(0.2),